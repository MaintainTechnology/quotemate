...
| **LLM conversation, deterministic money** | Roofing + painting SMS receptionists let Sonnet 5 drive the customer-facing turn; every figure still comes from a tool, checked by a grounding validator. | Shipped 2026-07-26 behind `SMS_LLM_RECEPTIONIST_ENABLED` (**default off**; `1` = all tenants, or a comma-separated tenant-id list). Supersedes "zero LLM in the roofing/painting customer flow" — `docs/strategy.md` v17. Fails open to the old state machines on any error. |
...
⚠ Changed 2026-07-26 (`docs/strategy.md` v17): the customer-facing flow is **no longer zero-LLM**. `app/api/sms/inbound/route.ts` `handleRoofingTurn` gates on `tenants.trades` containing `roofing` (a roofing-only tenant engages without a keyword), then decides the turn one of two ways:

- **`SMS_LLM_RECEPTIONIST_ENABLED` unset (the default)** — the **pure state machine** (`lib/sms/roofing-{intake,receptionist,compose}.ts`), byte-identical to before.
- **Flag on** — `claude-sonnet-5` drives the conversation (`lib/sms/llm-receptionist.ts`): greetings, questions, refusals, trade switches, and asking for missing details in natural language. It returns a **tool choice**, which maps onto the same `RoofingTurnDecision` union the route already switches on. Any throw, timeout, bad shape or grounding violation falls back to the state machine **for that turn**.
