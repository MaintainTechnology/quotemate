...
- **Pay-first:** the early-booking discount is realised at the **generic** `/r/[token]/[tier]` mint (`resolveMintDiscount`) — the flat-$99 roofing and painting mints carry no discount. `canTakePayment()` must gate every mint.
- **What the customer pays:** electrical/plumbing/solar = the tier deposit (or the $99 inspection when routed there); **roofing AND painting = the flat $99 refundable site visit and nothing else** — painting's 30% tier deposits are retired from every customer surface (`docs/strategy.md` v19), old `/r/paint/<token>/<G|B|B>` links 302 to `/r/paint/<token>/inspection`, and a held row 302s back to the quote page rather than paying anything.
