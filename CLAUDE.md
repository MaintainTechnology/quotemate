...
| **Auto-send vs review-required** | Path B: drafted quote auto-sends; tradie reviews after. | Live, per-trade nuance: electrical/plumbing/roofing **and residential painting** (`docs/strategy.md` v21, 2026-08-07) auto-send; **solar** auto-releases only *clean* estimates (`SOLAR_AUTO_RELEASE` default on — flagged/inspection held); **commercial painting** keeps its own stack/rule. ⚠ Price *visibility* only — orthogonal to what the customer is **charged**. See *review gates* below. |
...
**4. Painting — SMS receptionist + self-serve form, AUTO-SEND.**
Mirrors roofing exactly, including the LLM conversation layer and its `SMS_LLM_RECEPTIONIST_ENABLED` flag (**default ON**; `0` is the kill switch) (`lib/sms/painting-*.ts`, `handlePaintingTurn`), or the `/paint-request/[token]` form. Area from Google Solar building lookup + street-view; pricing in `lib/painting/pricing.ts`. **Auto-send since `docs/strategy.md` v21 (2026-08-07)**: a priced row is stamped `released_at` at save time in `runAndSavePaintingQuote` (`lib/painting/quote-dispatch.ts`) and the customer is texted the full quote on the same turn — tier prices, `/q/paint/[token]`, the PDF, the one $99 site-visit link. The tradie review gate is retired (it silently dropped 3 of 8 live sends); `notifyPaintingTradie` still fires, and `/p/[token]` + `/api/painting/release/[token]` remain for edits, resends and retrying a failed auto-send. ⚠ The gate modules are **unchanged and still load-bearing** — `canShowPaintingPrices` / `resolvePaintMintTier` still withhold an unreleased row, and a send that fails rolls `released_at` back (`revertPaintingRelease`) so nothing ever reports a delivery that did not happen. Inspection-routed rows are unchanged. Since v19 the ONLY customer payment is the flat $99 refundable visit — no tier deposit. Commercial painting is a **separate stack** (`lib/commercial-painting/*`, `paint_rates`/`paint_runs`) that texts the price immediately and is untouched by v21.
...
- **Who holds prices:** commercial-painting = held until tradie release; solar = held unless clean+auto-release; electrical/plumbing/roofing **and residential painting (v21)** = auto-send. Each trade has its own gate module — do not assume one covers another. ⚠ This is about **price visibility only** — it says nothing about what the customer is charged (see *What the customer pays* below); elec/plumb auto-send their prices AND take only the $99.
- **Two painting columns, two questions — never conflate them** (v21). `released_at` (mig 157) = *may the customer see prices*; written **before** the send because the quote page and the $99 mint gate on it, and a dashboard save stamps it while texting nobody. `quote_sent_at` (**mig 189**) = *did a carrier accept the message*; the ONLY thing `/p` may read to show "Sent to customer". Every painting send path returns `{ sent }`; a first send that fails rolls `released_at` back via `revertPaintingRelease`, which checks the write's `error` — **supabase-js resolves `{data, error}` on failure, it does not throw**, so a bare `await` on any Supabase or Twilio call is the silent-failure bug class this whole entry exists to close. Never re-defer the release send into `after()`, and never report `ok` without `sent`: that pairing is exactly how 3 of 8 live releases texted nobody while `/p` showed "Sent".
- **Pay-first:** the early-booking discount is realised at the **generic** `/r/[token]/[tier]` mint (`resolveMintDiscount`) — every flat-$99 mint carries no discount, so since v20 the discount is reachable **only by solar and commercial painting**; the elec/plumb quote page suppresses the countdown banner rather than advertise a saving they can no longer earn. `canTakePayment()` must gate every mint **on an `initial` row**. ⚠ Since `docs/strategy.md` v23 (mig 194) both this and the early-bird realisation are scoped to `quote_kind='initial'`: a post-site-visit `final`/`balance` child has no visit to book (the slots guard would refuse a legitimate deposit) and must never inherit an early-bird offer (it would silently discount the deposit). The child mint also skips the price hold and **refuses the `inspection` tier outright** — a second $99 would claim the child's only `paid_at` slot and permanently block the deposit.
- **What the customer pays** — the per-trade payment model, stated once:
  - **Flat $99 refundable site visit:** roofing, painting (`docs/strategy.md` v19), **electrical + plumbing** (v20, 2026-08-06). G/B/B prices stay **visible** as information; the price is confirmed on site. ⚠ "and nothing else" is no longer true for **electrical + plumbing**: since v23 (mig 194) the $99 is the FIRST of up to three charges on a job — the tradie then issues a `final` quote (deposit = a job-type percentage of the confirmed total, **less the $99 already paid**, plus a 2% platform fee charged on top) and requests a `balance` on completion. Each charge is its own `quotes` row (`quote_kind`, `parent_quote_id`). Roofing/painting are unchanged and still $99-only. Retired deposit links keep working by 302: `/r/paint/<token>/<G|B|B>` → `/r/paint/<token>/inspection`, and `/r/<token>/<G|B|B>` → `/r/<token>/inspection` for elec/plumb (`resolveGenericMintTier`, `lib/quote/mint-tier.ts`). A held painting row 302s back to the quote page rather than paying anything.
...
This project is indexed by GitNexus as **quotemax** (58004 symbols, 149183 relationships, 223 execution flows).
...
| Resource | Use for |
| --- | --- |
| `gitnexus://repo/quotemax/context` | Codebase overview, check index freshness |
| `gitnexus://repo/quotemax/clusters` | All functional areas |
| `gitnexus://repo/quotemax/processes` | All execution flows |
| `gitnexus://repo/quotemax/process/{name}` | Step-by-step execution trace |
