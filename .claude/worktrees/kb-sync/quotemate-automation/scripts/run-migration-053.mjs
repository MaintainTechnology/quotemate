// QuoteMate · run migration 053 (loader commit/rollback — trades + categories)
//
// Phase 2 — applied to the STAGING sandbox first:
//   node --env-file=.env.staging.local scripts/run-migration-053.mjs
// then to production at ship time, with explicit approval:
//   node --env-file=.env.local         scripts/run-migration-053.mjs
//
...
const sqlPath = join(here, "..", "sql", "migrations", "053_loader_commit_trades_categories.sql");
...
try {
  await c.connect();
  console.log(`→ Running 053_loader_commit_trades_categories.sql against ${target} (${sql.length.toLocaleString()} chars)...`);
  await c.query(sql);
  ...
  console.log(`\nOK — migration 053 verified on ${target}.`);
} catch (err) {
