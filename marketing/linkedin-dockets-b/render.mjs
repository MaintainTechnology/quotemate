// Render dockets-b.html to one PNG per slide, plus the swipeable carousel PDF that
...
for (const n of nums) {
  await page.goto(`http://localhost:${port}/dockets-b.html?d=${n - 1}`, { waitUntil: 'networkidle' })
...
if (wantPdf && !only) {
  ...
  await page.pdf({ path: join(OUT, 'quotemax-dockets-b.pdf'), width: `${W}px`, height: `${H}px`, printBackground: true })
