// Render grounding.html to one PNG per slide, plus the swipeable carousel PDF that
...
for (const n of nums) {
  await page.goto(`http://localhost:${port}/grounding.html?slide=${n - 1}`, { waitUntil: 'networkidle' })
  ...
  const buf = await page.screenshot({ path: join(OUT, `slide-${String(n).padStart(2, '0')}.png`) })
  shots.push(buf)
  console.log(`  ok slide-${String(n).padStart(2, '0')}.png`)
}
...
if (wantPdf && !only) {
  ...
  await page.pdf({ path: join(OUT, 'quotemax-grounding.pdf'), width: `${W}px`, height: `${H}px`, printBackground: true })
  console.log('  ok quotemax-grounding.pdf')
}

if (wantSheet) {
  const sheet = `<!doctype html><body style="margin:0;background:#0a0908;display:grid;grid-template-columns:repeat(5,1fr);gap:8px;padding:8px;font:700 22px 'JetBrains Mono',monospace;color:#FFC400">
    ${Array.from({ length: total }, (_, i) => `<div style="position:relative"><img src="out/slide-${String(i + 1).padStart(2, '0')}.png" style="width:100%;display:block"><span style="position:absolute;left:0;top:0;background:#000d;padding:3px 9px">${i + 1}</span></div>`).join('')}
  </body>`
