// Render leads.html to one PNG per slide, plus the swipeable carousel PDF that
...
for (const n of nums) {
  await page.goto(`http://localhost:${port}/leads.html?d=${n - 1}`, { waitUntil: 'networkidle' })
...
if (wantPdf && !only) {
  ...
  await page.pdf({ path: join(OUT, 'quotemax-leads.pdf'), width: `${W}px`, height: `${H}px`, printBackground: true })
