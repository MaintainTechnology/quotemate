  ...
  switch (step) {
    case "address": {
      ...
      if (addr) {
        ...
        const st2 = parseAuState(msg);
        if (st2) next.state = st2;
  ...
  if (lastStep && ANSWERABLE_STEPS.has(lastStep)) {
    ...
    if (!nextSlots.address) {
      ...
      if (addr) {
        ...
        const st2 = parseAuState(inbound);
        if (st2) nextSlots.state = st2;
...
// lib/sms/inbound-helpers.ts
var OPT_OUT_RE = /^\s*(stop|stopall|unsubscribe|cancel|end|quit)\s*[.!]*\s*$/i;
function isGlobalOptOut(body) {
  return OPT_OUT_RE.test(body ?? "");
}
function arrivalTimestampsFromTurns(rows2) {
  const list = rows2 ?? [];
  let lastOutboundIdx = -1;
  for (let i = list.length - 1; i >= 0; i--) {
    if (list[i]?.direction === "outbound") {
      lastOutboundIdx = i;
      break;
    }
  }
  const burst4 = list.slice(lastOutboundIdx + 1);
  const out = [];
  for (const r of burst4) {
...
// lib/sms/send-reliability.ts
var DELIVERY_KNOB_DEFAULTS = {
  maxDurationSec: 300,
  debounceMs: 1500,
  sendRetries: 2,
  sendBaseDelayMs: 500,
...
var KNOB_BOUNDS = {
  ...
  debounceMs: { min: 0, max: 6e4 },
  sendRetries: { min: 0, max: 10 },
  sendBaseDelayMs: { min: 0, max: 6e4 },
  sendMaxDelayMs: { min: 0, max: 12e4 }
};
function parseIntKnob(raw, fallback, bounds) {
...
function getDeliveryKnobs(env = process.env) {
...
function adaptiveDebounceMs(arrivalTimestamps, knobs2) {
  const base = Math.max(0, knobs2.debounceMs);
  const cap = base * 4;
...
// .scratch-audit/burst.ts
var knobs = getDeliveryKnobs({});
console.log("knobs", knobs);
function latestInbound(turns) {
  return [...turns].reverse().find((t) => t.direction === "inbound")?.body ?? "";
}
function coalesced(turns) {
  let lastOut = -1;
  for (let i = turns.length - 1; i >= 0; i--) if (turns[i].direction === "outbound") {
    lastOut = i;
    break;
  }
...
function show(label, prev, inbound) {
...
--- ${label}`);
  ...
  if (d) {
    ...
    if (d.action === "inspection") console.log("   reason    :", d.reason);
...
var afterAsk = { slots: {}, last_step: "address" };
var burst2 = [
...
for (const msg of ["hi", "i need a quote", "for my roof", "12 Smith St Bondi NSW 2026"]) {
  ...
  if (!d) {
    console.log("   (handed to general dialog)");
    continue;
  }
...
var rows = (gaps) => {
  ...
  for (const g of gaps) {
    t += g;
    out.push({ direction: "inbound", created_at: new Date(t).toISOString() });
  }
  return out;
};
for (const gaps of [[], [400, 400], [3e3, 3e3], [1400], [30]]) {
  const ts = arrivalTimestampsFromTurns(rows(gaps));
