  ...
  return (
    <section className="relative z-10 mx-auto mt-10 max-w-6xl px-6 pb-4 sm:px-10">
      <div className="flex flex-wrap items-center gap-3">
        <span className=" text-[0.8rem] font-semibold uppercase tracking-[0.08em] text-accent">
      ...
      <div className="rounded-card mt-6 border border-ink-line bg-ink-card p-6 sm:p-7">
        <div className=" text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-accent">Property details</div>
      ...
      {/* Paintable surfaces */}
      <div className="rounded-card mt-6 border border-ink-line bg-ink-card p-6 sm:p-7">
        <div className=" text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-accent">Paintable quantities</div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {measurement.surfaces.map((s) => (
            <div key={s.scope} className="rounded-card flex items-baseline justify-between border border-ink-line bg-ink-deep px-4 py-3">
              <span className=" text-sm font-semibold uppercase tracking-[0.08em] text-text-sec">{s.scope}</span>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        {price.tiers.map((t) => (
          <div key={t.tier} className="rounded-card border border-ink-line bg-ink-card p-6">
            ...
            <div className="mt-1 text-xs font-semibold uppercase tracking-[0.08em] text-text-dim">
      ...
      {price.manual_override && (
        <div className="rounded-card edge-lit mt-5 border border-ink-line bg-ink-card px-5 py-3 text-sm text-text-sec">
          <span className=" text-xs font-semibold uppercase tracking-[0.08em] text-accent">✎ Adjusted by you</span>{' '}
      ...
      {price.breakdown && (
        <div className="rounded-card edge-lit mt-6 border border-ink-line bg-ink-card p-6 sm:p-7">
          <div className=" text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-accent">How the price was built</div>
      ...
      {takeoff && (
        <div className="rounded-card edge-lit mt-6 border border-ink-line bg-ink-card p-6 sm:p-7">
          <div className=" text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-accent">Materials &amp; labour</div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            {takeoff.tiers.map((t) => (
              <div key={t.tier} className="rounded-card border border-ink-line bg-ink-deep p-4">
                <div className=" text-xs font-semibold uppercase tracking-[0.08em] text-text-sec">{t.tier}</div>
                <div className="mt-3 space-y-1.5 font-mono text-xs">
                  ...
                  {(t.products.some((p) => p.note) || t.labour_note) && (
                    <details className="mt-1.5">
                      <summary className="cursor-pointer list-item text-[11px] uppercase tracking-[0.08em] text-text-dim transition-colors hover:text-accent">
      ...
      <div className="rounded-card edge-lit mt-6 border border-ink-line bg-ink-card p-6 sm:p-7">
        <div className=" text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-accent">How this was derived</div>
...
function RoutingStrip({ routing }: { routing: PaintingRoutingDecision }) {
  ...
  return (
    <div className={`rounded-card mt-6 border ${warn ? 'border-warning-bright/40' : 'border-accent/40'} bg-ink-card px-6 py-5`}>
      <div className={` text-[0.78rem] font-semibold uppercase tracking-[0.08em] ${warn ? 'text-warning' : 'text-accent'}`}>
