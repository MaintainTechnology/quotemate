        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={`whitespace-nowrap border-b-2 px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] transition-colors ${
 current
...
export function Crumbs({ trail }: { trail: Array<{ label: string; href?: string }> }) {
  return (
    <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-3 text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-text-dim">
      {trail.map((c, i) => (
...
export function Label({ children, htmlFor }: { children: React.ReactNode; htmlFor?: string }) {
  const cls = 'mb-2 block text-xs font-semibold uppercase tracking-[0.08em] text-text-dim'
  if (htmlFor) {
...
export const BTN_PRIMARY =
  'inline-flex items-center justify-center gap-2 bg-accent px-6 py-3.5 text-sm font-semibold uppercase tracking-[0.08em] text-white transition-colors hover:bg-accent-press disabled:cursor-not-allowed disabled:opacity-50'

export const BTN_GHOST =
  'inline-flex items-center justify-center gap-2 border border-ink-line px-6 py-3.5 text-sm font-semibold uppercase tracking-[0.08em] text-text-pri transition-colors hover:border-accent hover:text-accent disabled:cursor-not-allowed disabled:opacity-50'

export const BTN_GHOST_SM =
  'inline-flex items-center justify-center gap-1.5 border border-ink-line px-3 py-1.5 text-[0.7rem] font-semibold uppercase tracking-[0.08em] text-text-sec transition-colors hover:border-accent hover:text-accent disabled:cursor-not-allowed disabled:opacity-50'

export const BTN_DANGER_SM =
  'inline-flex items-center justify-center gap-1.5 border border-ink-line px-3 py-1.5 text-[0.7rem] font-semibold uppercase tracking-[0.08em] text-text-dim transition-colors hover:border-warning-bright hover:text-warning-bright disabled:cursor-not-allowed disabled:opacity-50'
...
export function Tally({ label, value, tone }: { label: string; value: number; tone: Tone }) {
  return (
    <div className="rounded-card border border-ink-line bg-ink-card p-4 text-center">
      ...
      <div className="mt-1 text-[0.64rem] font-semibold uppercase tracking-[0.08em] text-text-dim">{label}</div>
    </div>
  )
}
    ...
    <div>
      ...
      {legend && (
        <div className="mt-2.5 flex flex-wrap gap-x-5 gap-y-1.5" aria-hidden="true">
          {visible.map((s) => (
            <span key={s.key} className="inline-flex items-center gap-2 text-[0.66rem] font-semibold uppercase tracking-[0.08em] text-text-dim">
  ...
  return (
    <div>
      ...
      <div className={`rounded-card mt-5 border border-ink-line bg-ink-card p-5 ${REVEAL}`} style={delay(300)}>
        <div className="mb-3 text-xs font-semibold uppercase tracking-[0.08em] text-text-dim">Fleet health</div>
...
export function NumberedEyebrow({ n, children }: { n: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className=" text-xs font-semibold uppercase tracking-[0.08em] text-text-dim">{children}</span>
    </div>
  )
}
...
export function EmptyState({ title, body, children }: { title: string; body: string; children?: React.ReactNode }) {
  return (
    <div className="rounded-card border border-dashed border-ink-line bg-ink-card/50 px-7 py-10 text-center">
      <div className=" text-xs font-semibold uppercase tracking-[0.08em] text-text-dim">{title}</div>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        type="button"
        onClick={onClose}
        className="rounded-card absolute right-6 top-6 border border-ink-line bg-ink-card px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-text-pri transition-colors hover:border-accent hover:text-accent"
      >
        Close ✕
      </button>
    </div>
