...
import { buildConversationFollowup2hSms, buildFollowup2hSms } from './templates'
...
// ─── Migration 159 — mid-conversation check-in template ──────────────

describe('buildConversationFollowup2hSms — body shape', () => {
  it('produces the exact AU-tone body with firstName and businessName interpolated', () => {
    ...
    expect(body).toBe(
      "Hi Sam, just checking in - still keen to get that job sorted? Reply here and we'll pick up where we left off. - Atomic Electrical",
    )
  })

  it('does NOT reference a quote (there is no quote yet mid-intake)', () => {
    const body = buildConversationFollowup2hSms({
      firstName: 'Sam',
      businessName: 'Atomic Electrical',
    })
    expect(body.toLowerCase()).not.toContain('quote we sent')
  })

  it("firstName falsy/empty falls back to 'there'", () => {
    expect(
      buildConversationFollowup2hSms({ firstName: '', businessName: 'Atomic Electrical' }),
    ).toContain('Hi there,')
    expect(
      buildConversationFollowup2hSms({ firstName: null, businessName: 'Atomic Electrical' }),
    ).toContain('Hi there,')
    expect(
      buildConversationFollowup2hSms({
        firstName: undefined,
        businessName: 'Atomic Electrical',
      }),
    ).toContain('Hi there,')
  ...
  it("businessName falsy falls back to 'your tradie' (defensive)", () => {
    expect(
      buildConversationFollowup2hSms({ firstName: 'Sam', businessName: '' }),
    ).toContain('- your tradie')
    expect(
      buildConversationFollowup2hSms({ firstName: 'Sam', businessName: null }),
    ).toContain('- your tradie')
  })
...
describe('buildConversationFollowup2hSms — GSM-7 length', () => {
  it('body length is <=160 chars with typical names (single SMS segment)', () => {
    const body = buildConversationFollowup2hSms({
      firstName: 'Sam',
      businessName: 'Atomic Electrical',
